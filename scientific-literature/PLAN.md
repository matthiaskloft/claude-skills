...
All 5 BayesFlow repos share the same `CLAUDE.md` mandate: verify references via OpenAlex, use APA 7 formatting, and keep implementations source-backed. Only `bayesflow_irt_article` has real infrastructure (7 Python scripts, 2 agents, 1 orchestrator skill) and `bayesflow_hpo` has a well-maintained `references.md`. The other 3 repos have zero tooling. Existing code is entirely hardcoded to IRT topics, so nothing is reusable across repos.
...
| Reference store | `_index.json` as source of truth + auto-generated `references.md` and `references.bib` |
...
| LaTeX safety | `references.bib` is derived only after normalization and validation; compile smoke tests are optional |
...
└── scientific-literature/
    ...
    │   ├── reference_index.py
    │   └── bibtex.py                     # BibTeX/LaTeX-safe export + validation
    ...
    │   └── bibtex-reference.md               # BibTeX export, escaping, validation rules
...
| `/lit-latex` | Export LaTeX-safe `references.bib` from `_index.json`, lint BibTeX formatting, resolve citekeys, and optionally smoke-test compilation. | Explicit or "make/update/check the bib file" |
...
def extract_work_info(work) -> dict
```
...
```python
def format_apa7(metadata: dict) -> str
...
def build_category_extract(sections, target_sections) -> list[dict]
```

### `lib/pipeline_state.py`
**Source:** `bayesflow_irt_article/litreview/scripts/pipeline_state.py`; parameterized state file path
...
### `lib/zotero_local.py`
**Source:** `a6_acquire_fulltexts.py:57-129` plus new SQLite support
...
class ZoteroSearch:
    ...
    def _search_sqlite(self, title, doi) -> tuple[str | None, dict]
    def _scan_pdfs(self, title) -> tuple[str | None, float]
```
...
class ReferenceIndex:
    ...
    def to_references_md(self, output_path)
```

### `lib/bibtex.py`
**Source:** New; derives LaTeX-safe BibTeX from normalized metadata in `_index.json`

```python
def infer_bibtex_type(metadata: dict) -> str
def sanitize_citekey(citekey: str) -> str
def escape_latex(text: str, mode: str = "bibtex") -> str
def protect_title_case(title: str) -> str
def format_bibtex_authors(authors: list[str]) -> str
def metadata_to_bibtex_entry(citekey: str, metadata: dict) -> str
def validate_bibtex_entry(citekey: str, metadata: dict) -> list[str]
def validate_bibtex_file(contents: str) -> list[str]
```

Design constraints:
- `_index.json` citekeys remain canonical; `references.bib` is derived output only.
- Export is safety-first: write to a temp `.bib`, validate, then replace the real file.
- Validation catches common compile failures before write: unescaped special characters, malformed author lists, unbalanced braces, invalid citekeys, and missing required fields for known entry types.
- `smoke-compile` is optional and only runs when a LaTeX toolchain is available.
...
### `references.bib` (auto-generated from `_index.json`)
BibTeX export for LaTeX manuscripts. Generated by `scripts/latex.py` via `lib/bibtex.py`, never hand-edited.

Compilation-safety policy:
- Auto-fix where deterministic: LaTeX escaping, citekey sanitization, whitespace cleanup, title-case protection for acronyms and proper nouns.
- Hard fail on likely breakage: unbalanced braces, malformed `author` fields, invalid or duplicate citekeys, missing required fields for inferred entry type.
- Warn on suspicious but non-fatal issues: ambiguous entry type, Unicode that may require BibLaTeX/XeLaTeX, sparse metadata for preprints.
...
| BibTeX export/lint | Zero tokens (pure Python scripts) |
...
- `/lit-plan` generates `litreview.yaml` and `litreview/*.yaml` from the hardcoded config in the old scripts
- `_index.json` generated from existing `docs/references/` markdown files
- `references.bib` generated later from `_index.json` so Markdown and LaTeX citekeys remain identical
...
4. `lib/bibtex.py` + `scripts/latex.py` (LaTeX-safe export and validation)
5. `lib/zotero_local.py` + `lib/arxiv_client.py` (acquisition)
6. `lib/pdf_extract.py` (extraction)
7. `lib/pipeline_state.py` (orchestration)
8. `scripts/` CLI wrappers
9. `templates/` for config-heavy scripts
10. Core skills: `/lit-guide`, `/lit-plan`, `/lit-validate`, `/lit-cite`, `/lit-latex`, `/lit-search`, `/lit-acquire`
11. Pipeline skills: `/lit-extract`, `/lit-crawl`, `/lit-status`, `/lit-run`
12. Agents: `lit-abstract-reviewer`, `lit-triager`
13. Update `marketplace.json` in `.claude-plugin/`
14. Migration: `/lit-plan` for `bayesflow_irt_article` (generates config from old scripts)
...
2. `/lit-cite 10.1145/3292500.3330701` produces APA 7 output matching the manual version; `authors`, `authors_str`, and `apa7` are correct.
3. `/lit-acquire` for a Zotero-held paper finds it via SQLite, copies the PDF, and updates `_index.json` with the `pdf` path.
4. Full pipeline for `bayesflow_irt_article` A6-A8 produces output consistent with existing artifacts.
5. Auto-generated `references.md` from `_index.json` diffs cleanly against existing `bayesflow_hpo/docs/references.md`.
6. `/lit-latex export-bib` on an existing `_index.json` writes `references.bib` only after validation succeeds.

### Failure / edge cases
7. `/lit-validate` on an unknown DOI returns not-found cleanly, with no crash and no partial write to `_index.json`.
8. `/lit-cite` for a paper already in `_index.json` detects the duplicate via `find_by_doi` and returns the existing entry without overwrite.
9. APA 7 formatting for a preprint (arXiv) and a book chapter uses the correct template by publication type.
10. `/lit-acquire` for a paper not in Zotero and not OA exhausts all fallbacks and reports failure with an actionable message.
11. `generate_citekey` collision for two papers with the same first-author last name and year is disambiguated with `a`/`b` suffixes.
12. `/lit-latex lint-bib` catches unescaped `%`, `&`, `_`, malformed `author` values, and unbalanced braces before file replacement.
13. `/lit-latex export-bib` for an arXiv-only entry emits valid preprint BibTeX and warns if metadata is sparse.
14. `/lit-latex smoke-compile` succeeds on a generated temporary document with `\nocite{*}` when a TeX toolchain is installed, and downgrades to static validation when it is not.

### Non-IRT bootstrap
15. `/lit-plan` run from scratch in `bayesflow_hpo` scaffolds `litreview.yaml`, `litreview/`, `_index.json`, and optional `references.bib` generation from existing `docs/references.md`, with no IRT-specific config required.
...
|---|----------|---------|------------|
| 1 | High | No `agents/` directory or discovery story | Added `agents/` to plugin structure; agents section specifies path resolution via `<plugin-path>/agents/<name>.md` |
| 2 | High | `_index.json` authors stored as string, not list; pipe-delimited source; single category | Schema updated: `authors` is now a list, `authors_str` holds APA-formatted string, `source` is a list, and `categories` is a list |
| 3 | Medium | WSL paths implied but not scoped | Explicitly scoped to WSL POSIX paths only; native Windows paths not supported |
| 4 | Medium | Verification too narrow; no failure cases or non-IRT bootstrap | Expanded to cover failure modes, APA edge cases, duplicate handling, LaTeX validation, and `bayesflow_hpo` bootstrap |
| 5 | Low | Skill count said 11 but only 10 defined | Corrected by adding `/lit-latex` and updating the skill inventory |
